...
class ExternalEquilibriumMoistureContent:
...
class FitGrid:
    def __init__(self, emc):
        assert isinstance(emc, ExternalEquilibriumMoistureContent), 'emc must be an EEMC'
        self.emc = emc
...
if __name__ == '__main__':
    # Example: Read in an EMC and get the coefficients
    corn_file = '/home/kyle/repos/AgAlarm/EMCfitting/soybeanEMC.csv'
    corn_emc = ExternalEquilibriumMoistureContent(corn_file)
    corn_fit = FitGrid(corn_emc)
    corn_fit.check_fit()
    print(corn_fit.fit)

    # Example: Create EMC value from a given temperature/humidity
    measurement = ResultantEquations(57, .66)
    print(measurement.soybean_equation())
